...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template priority="10" match="w:rPr/w:b[@w:val=('0','none','false')]">
    <span style="font-weight: normal">
      <xsl:call-template name="tuck-next"/>
    </span>
  </xsl:template>

  <xsl:template priority="10" match="w:rPr/w:i[@w:val=('0','none','false')]">
    <span style="font-style: normal">
      <xsl:call-template name="tuck-next"/>
    </span>
  </xsl:template>

  <xsl:template priority="10" match="w:rPr/w:u[@w:val=('0','none','false')]">
...
<!-- Note italics, bold and underline are dropped except when set in a style. They are picked
  ...
  <!-- blip is "binary large image or picture" http://officeopenxml.com/drwPic-ImageData.php -->
  <xsl:template match="a:blip">
    <img>
      <!-- If we can, we will grab the target of an external link -->
      <xsl:for-each select="key('rels-by-id',@r:embed,$relations-doc)">
        <xsl:attribute name="src" select="concat($docx-base,@Target)"/>
      </xsl:for-each>
    </img>
  </xsl:template>
